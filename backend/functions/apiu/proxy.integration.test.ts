...
describe('worker api proxy (apiu alias)', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('proxies /apiu/auth to /api/auth upstream and preserves cookies', async () => {
    ...
    const request = new Request('https://auth.chefgroep.nl/apiu/auth', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Cookie: 'session=old',
  ...
  it('strips sensitive headers on public non-auth routes', async () => {
    ...
    const request = new Request('https://auth.chefgroep.nl/apiu/public/health', {
      method: 'GET',
      headers: {
        Cookie: 'session=old',
        Authorization: 'Bearer token',
  ...
  it('redirects non-public unauthenticated paths to auth origin', async () => {
    ...
    const request = new Request('https://auth.chefgroep.nl/apiu/agents');
